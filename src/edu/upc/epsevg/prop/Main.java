...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //NinePuzzle goal = new NinePuzzle(new int[]{1, 3, 5, 4, 2, 8, 6, 7, 0},null);
        //NinePuzzle start = new NinePuzzle(new int[]{1, 2, 3, 4, 5, 0, 6, 7, 8}, goal);
        
        // puzzle amb solució de 31 moviments
        NinePuzzle goal = new NinePuzzle(new int[]{1, 2, 3, 4, 5, 6, 7, 8, 0}, null);
        NinePuzzle start = new NinePuzzle(new int[]{8, 6, 7, 2, 5, 4, 3, 0, 1}, goal); 
        System.out.println(start.getHeuristica());
        ...
        cercaInformada(start, goal, TipusCerca.HILL_CLIMBING, 32);
    }
    ...
    private static void cercaInformada(NinePuzzle start , NinePuzzle goal, TipusCerca tipusCerca, int max_depth) {
        ...
        do {
            ...
            while (!solutionReached && LNO.size() > 0) {
                ...
                // d'anar netejant de la memòria les regions de l'arbre ja explorades
                // i que no són solució.
                if (tipusCerca != TipusCerca.HILL_CLIMBING) {
                    NinePuzzle p = g.getParent(currentPuzzle);
                    if (p != null) {
                        int pe = g.getParentEdge(currentPuzzle);                        
                    ...
                    if (    !isCycle(g, f, currentPuzzle) && 
                        ...
                        if (f.isSolution(goal)) {
                            ...
                            if (tipusCerca == TipusCerca.HILL_CLIMBING) {
                                // A la cerca en amplada, l'afegim al final.
                                LNO.add(f);
                                Collections.sort(LNO);
                                NinePuzzle p = LNO.get(0);
                                LNO.clear();
                                LNO.add(p);
                                // afegim el node al graf
                                if (!g.containsVertex(p)) g.addChild(e++, currentPuzzle, p);
                            } else {
                                // A la cerca en profunditat, l'afegim al començament.
                                LNO.add(pos++, f);
                            }
